...
#data$Taxa<-as.factor(paste0(data$Clade,data$Maturity,sep=" "))
...
## Removing columns that have more than 100 missing value
...
## Subtracting the observations for the Taxa that have at least observations


```{r}
lennn<-(ncol(data1_cleaned)-2)/2
taxa1<-table(data1_cleaned$Clade)
...
data1_cleanedd$Clade<-data1_cleanedd$Var1

data1_cleaned1<-data1_cleanedd[data1_cleanedd$Freq>lennn,]

data1_cleaned<-data1_cleaned[data1_cleaned$Clade%in%unique(data1_cleaned1$Clade),]
...
category_counts <- table(data_log$Clade)
...
```{r}
write.csv(data_log,"teeth_data_log_clade_epoch.csv", row.names = FALSE)
write.csv(data_original,"teeth_data_clade_epoch.csv", row.names = FALSE)
```
